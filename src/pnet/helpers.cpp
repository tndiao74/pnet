...
std::string System::exec(const std::string& cmd)
{
    ...
    if (CreatePipe(&g_hChildStd_OUT_Rd, &g_hChildStd_OUT_Wr, &saAttr, 0))
    {
        ...
        _szcpy((char*)Buf::buf0, Buf::bufMax, cmd.c_str());
        TCHAR* szCmdline = (TCHAR*)(Buf::buf0);
        ...
        if (bSuccess)
        {
            ...
            for (;;)
            {
                if (Buf::buf1Len >= Buf::bufMax) break;
                bSuccess = ReadFile(g_hChildStd_OUT_Rd, Buf::buf1, Buf::bufMax, &dwRead, NULL);
                if (!bSuccess || dwRead == 0) break;
                Buf::buf1Len += dwRead;
            }
            if (Buf::buf1Len < Buf::bufMax)
                Buf::buf1[Buf::buf1Len] = 0;
            res += (const char*)Buf::buf1;
        }
    }
    return res;
}
